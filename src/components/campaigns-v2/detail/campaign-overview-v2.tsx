      ...
      <CardContent className="flex flex-col gap-2">
        <div className="flex items-center gap-3">
          ...
          <div className="min-w-0">
            ...
            <p className="text-lg font-semibold font-headline tracking-tight">
              {value}
            </p>
            {subtitle && (
        ...
        {progressValue !== undefined && (
          <ProgressBar value={progressValue} hideValue />
        )}
      </CardContent>
    </Card>
  );
...
export function CampaignOverviewV2({ campaign }: CampaignOverviewV2Props) {
  const { metrics, trigger, channels, audienceSegments, audienceSize } =
    campaign;
    ...
    <div className="space-y-6 pt-4">
      <div className="grid grid-cols-2 gap-3 lg:grid-cols-5">
        ...
        <MetricCard
          ...
          icon={TrendingUp}
          progressValue={hasMetrics ? metrics.responseRate : undefined}
        />
        <MetricCard
          label="Conversion Rate"
          value={hasMetrics ? `${metrics.conversionRate}%` : "—"}
          icon={TrendingUp}
          progressValue={hasMetrics ? metrics.conversionRate : undefined}
        />
        <MetricCard
          label="Revenue"
        ...
        <Card className="border border-border shadow-sm">
          ...
          <CardContent className="space-y-2">
            <div className="flex items-center justify-between">
              ...
              <span className="text-sm font-medium">
                {audienceSegments.length} filter
                {audienceSegments.length !== 1 ? "s" : ""}
              </span>
            </div>
            <div className="flex items-center justify-between">
