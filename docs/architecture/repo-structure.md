...
| `src/lib/nav-visibility/` | **Sidebar navigation visibility**: `nav-visibility-storage.ts` (localStorage key `sm-nav-visibility-v1`; stores hidden nav labels per product), `nav-visibility-provider.tsx` (`useNavVisibility`). Wrapped in root `layout.tsx`. Used by General Settings to let users show/hide sidebar pages per application; consumed in `page.tsx` to filter nav items before passing to `<Sidebar>`. |
